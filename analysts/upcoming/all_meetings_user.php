...
<div class="container-fluid">
   ...
   <div class="row-fluid" style="margin-bottom:20px;">
    <div class="span6"> 
      <a class="btn <?php echo (!isset($type) || $type == 0)?'big':''; ?>" style="" href="?mon=<?php echo $month;?>&amp;yr=<?php echo $year;?>&amp;com_type=<?php echo encrypt($com_type); ?>">All</a>
      <a class="btn meet1_big <?php echo ($type == 1)?'big':''; ?>" style="color:#fff;" href="?mon=<?php echo $month;?>&amp;yr=<?php echo $year; ?>&amp;type=<?php echo encrypt('1') ?>&amp;com_type=<?php echo encrypt($com_type); ?>">AGM</a>
      <a class="btn meet2_big <?php echo ($type == 2)?'big':''; ?>" style="color:#fff;" href="?mon=<?php echo $month;?>&amp;yr=<?php echo $year; ?>&amp;type=<?php echo encrypt('2') ?>&amp;com_type=<?php echo encrypt($com_type); ?>">EGM</a>
      <a class="btn meet3_big <?php echo ($type == 3)?'big':''; ?>" style="color:#fff;" href="?mon=<?php echo $month;?>&amp;yr=<?php echo $year; ?>&amp;type=<?php echo encrypt('3') ?>&amp;com_type=<?php echo encrypt($com_type); ?>">PB</a>
      <a class="btn meet4_big <?php echo ($type == 4)?'big':''; ?>" style="color:#fff;" href="?mon=<?php echo $month;?>&amp;yr=<?php echo $year; ?>&amp;type=<?php echo encrypt('4') ?>&amp;com_type=<?php echo encrypt($com_type); ?>">CCM</a>
        ...
        <div class="span6" align="right"> 
          <div class="btn-group">
            ...
            <a class="btn blue <?php echo ($com_type == 7)?'active':''; ?>" style="color:#fff;" href="?mon=<?php echo $month;?>&amp;yr=<?php echo $year; ?>&amp;type=<?php echo encrypt($type) ?>&amp;com_type=<?php echo encrypt('7') ?>">Wishlist</a>
          </div>
                        ...
                        <div style="min-height:130px;">
                           ...
                           <?php 
                              ...
                              if($total_sub != 0) {
                                 ...
                                 $query = mysql_query("SELECT proxy_ad.id from proxy_ad inner join companies on proxy_ad.com_id = companies.com_id  where proxy_ad.com_id IN (".$str_comp.") and  proxy_ad.meeting_date = '$date' order by proxy_ad.meeting_type asc ");
                                 while ($row = mysql_fetch_array($query)) {
                                    array_push($proxy_ids, $row["id"]);
                                 }
                              }
                              
                              $query = mysql_query("SELECT proxy_ad.id from proxy_ad inner join manual_subscription on proxy_ad.com_id = manual_subscription.report_id  where manual_subscription.report_type='1' and manual_subscription.user_id='$user_id' and  proxy_ad.meeting_date = '$date'  ");
                              while ($row = mysql_fetch_array($query)) {
                              ...
                              if(sizeof($proxy_ids) < 5 || ( isset($type) && $type != 0 )){
                                 foreach ($proxy_ids as $proxy_id) {
                                    $sql_p = mysql_query("SELECT companies.com_name, proxy_ad.meeting_type from proxy_ad inner join companies on proxy_ad.com_id = companies.com_id where proxy_ad.id='$proxy_id' ");
                                    $row_p = mysql_fetch_array($sql_p);
                                    echo '<div class="meet meet'.$row_p["meeting_type"].'" data-toggle="modal" href="#stack1" onclick="view_report(\''.$row_p["com_name"].'\','.$proxy_id.',1);">'.$row_p["com_name"].'</div>';
                                 }
                              } else {
                                 $str_proxy = implode(',', $proxy_ids);
                                 echo '<input type="hidden" value="'.$str_proxy.'" id="proxy_id_fetch_'.$count_day.'">';

                                 echo '<div data-toggle="modal" href="#myModal" onclick="view_compact(\''.date("d-M-y", $date).'\','.$count_day.');">';

                                 $sql_p = mysql_query("SELECT id from proxy_ad  where meeting_type='1' and id IN (".$str_proxy.") ");
                                 echo '<div class="meet meet1">'.mysql_num_rows($sql_p).' AGM</div>';

                                 $sql_p = mysql_query("SELECT id from proxy_ad  where meeting_type='2' and id IN (".$str_proxy.") ");
                                 echo '<div class="meet meet2">'.mysql_num_rows($sql_p).' EGM</div>';

                                 $sql_p = mysql_query("SELECT id from proxy_ad  where meeting_type='3' and id IN (".$str_proxy.") ");
                                 echo '<div class="meet meet3">'.mysql_num_rows($sql_p).' PB</div>';
                                  $sql_p = mysql_query("SELECT id from proxy_ad  where meeting_type='4' and id IN (".$str_proxy.") ");
                                 echo '<div class="meet meet4">'.mysql_num_rows($sql_p).' CCM</div>';
...
<!-- Modal -->
<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="width:30%; margin-left:-15%">
...
function view_report(company_name, proxy_id, report_type){
   $("#stack1 .modal-header").html('<div class="row-fluid"><div class="span6"><h3>'+company_name+'</h3></div><div class="span6" style="text-align:right"><a href="javascript:;" role="button" class="btn blue" onclick="view_request(\''+company_name+'\',' + proxy_id +' )" >Proxy Request</a>&nbsp;<a href="javascript:;" role="button" class="btn blue" onclick="view_vote_upcoming(\''+company_name+'\',' + proxy_id +' )" >Vote</a>&nbsp;<a href="javascript:;" role="button" class="btn yellow" onclick="view_report(\''+company_name+'\',' + proxy_id +',1 )" ><i class="icon-chevron-left"></i> Report</a></div></div>'); 
