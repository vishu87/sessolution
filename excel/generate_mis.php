...
foreach ($proxy_ids as $proxy_id) {
	...
	foreach ($user_proxy_array as $user_id) {
		...
		$query_res = mysql_query("SELECT voting.id, voting.resolution_name, voting.resolution_number, voting.man_reco, voting.man_share_reco, ses_recos.reco from voting inner join ses_recos on voting.ses_reco = ses_recos.id where voting.report_id='$proxy_id' order by voting.priority, voting.resolution_number asc "); 
