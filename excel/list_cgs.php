...
$date_sql = '';
  ...
  if($date_high && $date_low){
   $date_sql = "and cgs.publishing_date <= '$date_high' and cgs.publishing_date >= '$date_low' ";
  } elseif ($date_high && !$date_low) {
    $date_sql = "and cgs.publishing_date <= '$date_high' ";
  } elseif(!$date_high && $date_low) {
    $date_sql = "and cgs.publishing_date >= '$date_low' ";
  }

  $sql = mysql_query("SELECT cgs.*, companies.com_name,companies.com_bse_code from cgs inner join companies on cgs.com_id = companies.com_id ".$date_sql." order by cgs.publishing_date desc" );
...
$ar_fields = array("sn","com_bse_code","com_name","publishing_date","report_upload","sub","govt_index","india_man");

$ar_names = array("SN","BSE Code","Company Name","Publishing Date","Report","Subscribers","Govt. Index","India Mandatory");
...
while($row = mysql_fetch_array($sql))
{
	...
	foreach ($ar_fields as $ar) {
      ...
      $sql_pack_user = mysql_query("SELECT users_package.user_id from users_package inner join package on users_package.package_id = package.package_id inner join package_company on package_company.package_id = package.package_id where package_company.com_id = '$row[com_id]' and package.package_year='$row[year]' and package.package_type = '2' ");
       ...
       $sql_sub = mysql_query("SELECT user_id from users_companies where com_id='$row[com_id]' and type='2' and year='$row[year]' ");


      $var =mysql_num_rows($sql_pack_user) + mysql_num_rows($sql_sub);
		}
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'Corporate Governance Scores');
...
$name = 'CGS_'.date("d-M-y",strtotime("now"));
