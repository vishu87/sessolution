...
$index = 0;
$sql_port = mysql_query("SELECT * from user_portfolio where user_id = '$_SESSION[MEM_ID]' order by portfolio_name asc ");
while($row = mysql_fetch_array($sql_port)) {

$sql = "SELECT companies.com_name, companies.com_isin, companies.com_bse_code from portfolio_companies inner join companies on portfolio_companies.com_id = companies.com_id where portfolio_companies.portfolio_id='$row[id]' order by companies.com_name asc";
...
$objPHPExcel->getActiveSheet()->setCellValue(chr($cell_val).$seq, $var);
...
$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Companies in '.$row["portfolio_name"]);

$objPHPExcel->getActiveSheet()->setTitle($row["portfolio_name"]);
