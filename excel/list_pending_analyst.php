  ...
  $year_sql = mysql_query("SELECT year_sh from years order by year_sh desc");
    while ($year_row = mysql_fetch_array($year_sql)) {
      ...
      if(sizeof($total_comp) > 0){
        ...
        $sql = mysql_query("SELECT id from proxy_ad where com_id IN (".$str_comp.") and completed_on = '' and year ='$year' ".$str_time_1." order by meeting_date desc");
...
$objPHPExcel->setActiveSheetIndex(0);
 $ar_fields = array("sn","com_bse_code","com_name","meeting_date","meeting_type","deadline","critical","subscribed","report","notice","data","d_deadline","d_completed","analysis","a_deadline","a_completed","review","r_deadline","r_completed");

$ar_names = array("SN","BSE Code","Company Name","Meeting Date","Meeting Type","Deadline","Critical/Upcoming","Subscribed","Report","Notice","Data","Deadline","Completed On","Analysis","Deadline","Completed On","Review","Deadline","Completed On");
...
{
     $sql = mysql_query("SELECT proxy_ad.*, companies.com_name, companies.com_bse_code from proxy_ad inner join companies on proxy_ad.com_id = companies.com_id  where proxy_ad.id='$proxy_id' ");
     $row = mysql_fetch_assoc($sql);

$sql_data = mysql_query("SELECT an_id, deadline, completed_on from report_analyst where report_id= '$proxy_id' and rep_type='1' and type= '1' ");
$data = mysql_fetch_array($sql_data);
$sql_analysis = mysql_query("SELECT an_id, deadline, completed_on from report_analyst where report_id= '$proxy_id' and rep_type='1' and type= '2' ");
$analysis = mysql_fetch_array($sql_analysis);
$sql_review = mysql_query("SELECT an_id, deadline, completed_on from report_analyst where report_id= '$proxy_id' and rep_type='1' and type= '3' ");
...
if($color != '')
$objPHPExcel->getActiveSheet()->getStyle('Q'.$seq.':S'.$seq)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB($color);
$seq++;
}
}
$objPHPExcel->getActiveSheet()->freezePane('D3');
$objPHPExcel->getActiveSheet()->mergeCells('A1:'.$val_chr.'1');
$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Pending Proxy Advisory Reports');
...
$objPHPExcel->setActiveSheetIndex(1);
  ...
  $sql_cgs = "SELECT cgs.*, companies.com_name,companies.com_bse_code from cgs inner join companies on cgs.com_id = companies.com_id  and cgs.completed_on='' ".$str_time_2." ";

$result_att=mysql_query($sql_cgs);

$ar_fields = array("sn","com_bse_code","com_name","publishing_date","deadline","critical","subscribed","report_upload","data","d_deadline","d_completed","analysis","a_deadline","a_completed","review","r_deadline","r_completed");

$ar_names = array("SN","BSE Code","Company Name","Meeting Date","Deadline","Critical/Upcoming","Subscribed","Report","Data","Deadline","Completed On","Analysis","Deadline","Completed On","Review","Deadline","Completed On");
$ar_width = array("6","20","20","20","20","20","20","20","20","20","20","20","20","20","20","20","20","20","20");
...
$color = check_status($data["deadline"],$data["completed_on"]);
if($color != '')
$objPHPExcel->getActiveSheet()->getStyle('I'.$seq.':K'.$seq)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB($color);
$color = check_status($analysis["deadline"],$analysis["completed_on"]);
if($color != '')
$objPHPExcel->getActiveSheet()->getStyle('L'.$seq.':N'.$seq)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB($color);
$color = check_status($review["deadline"],$review["completed_on"]);
if($color != '')
$objPHPExcel->getActiveSheet()->getStyle('O'.$seq.':Q'.$seq)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB($color);
...
$objPHPExcel->getActiveSheet()->freezePane('D3');
$objPHPExcel->getActiveSheet()->mergeCells('A1:'.$val_chr.'1');
$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Pending Governance Scores');
...
$objPHPExcel->setActiveSheetIndex(2);
  ...
  $sql_cgs = "SELECT research.*, companies.com_name,companies.com_bse_code from research inner join companies on research.com_id = companies.com_id  and research.completed_on='' ".$str_time_3." ";
...
$objPHPExcel->getActiveSheet()->freezePane('D3');
$objPHPExcel->getActiveSheet()->mergeCells('A1:'.$val_chr.'1');
$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Pending Governance Research');
$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->getColor()->setARGB('FF777777');
$objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setSize(14); 
...
$name = 'Analyst_'.date("d-M-y", strtotime("now"));
