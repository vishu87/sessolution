...
function send_email($change_fields,$com_id,$proxy_id){
	...
	$rep_sql = mysql_query("SELECT companies.com_name, proxy_ad.* from proxy_ad inner join companies on proxy_ad.com_id = companies.com_id where companies.com_id='$com_id' and proxy_ad.id='$proxy_id' ");
	...
	$count = 0;
	$body_in = '<p>Updates for <b>'.$com_name.'</b> / <b>'.$meeting_types[$row_rep["meeting_type"]].'</b> / <b>'.date("d-M-y",$row_rep["meeting_date"]).'</b></p>';
