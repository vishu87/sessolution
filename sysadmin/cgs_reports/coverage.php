	...
	<div class="row-fluid search-forms search-default">
            ...
            <div class="portlet box light-grey">
                     <div class="portlet-title">
                        <h4><i class="icon-globe"></i>CGS Coverage (<?php echo $fetch_period[$search_year];?>)</h4>
                     </div>
                     <?php
                       $total_comp = array();

                        $sql_report = mysql_query("SELECT distinct package_company.com_id from package_company inner join package on package_company.package_id = package.package_id where package.package_year = '$search_year' and package_type='2' ");
                        while($row_cgs = mysql_fetch_array($sql_report)){
                          array_push($total_comp, $row_cgs["com_id"]);
                        }

                         $sql_report = mysql_query("SELECT distinct com_id from users_companies where type='2' and year = '$search_year' ");
                        while($row_cgs = mysql_fetch_array($sql_report)){
                         ...
                         $sql_report = mysql_query("SELECT distinct com_id from cgs where year = '$search_year' ");
                        while($row_cgs = mysql_fetch_array($sql_report)){
                          if(!in_array($row_cgs["com_id"], $total_comp))
                            array_push($total_comp, $row_cgs["com_id"]);
                        }
