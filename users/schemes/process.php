...
if($_GET["cat"] == 1){	
		...
		if(mysql_query("INSERT into schemes (scheme_name, dp_id, client_id, user_id, created_at) values ('$_POST[scheme_name]', '$_POST[dp_id]', '$_POST[client_id]', '$_SESSION[MEM_ID]', '$id') ")){
	...
	if($_GET["update"]){
		...
		if(is_numeric($update_id)) {
			...
			if(mysql_num_rows($query) == 0){
				...
				if(mysql_query("UPDATE schemes set scheme_name = '$scheme_name', dp_id = '$dp_id', client_id = '$client_id' where id = '$update_id' ")) header("Location: ../".$folder.".php?cat=2&update=".encrypt($update_id)."&success=1");
